...
func (s *podmanService) Stop() (output string, err error) {
	output, err = s.Execer.RunCommandAndReturn(SystemCtl, "", "stop", s.id)
	return
}
...
func (s *podmanService) installService() (output string, err error) {
	...
	if err == nil {
		var result string
		result, err = s.Execer.RunCommandAndReturn("mv", "", s.id, "/etc/systemd/system")
		if err == nil {
			output = fmt.Sprintf("%s\n%s", output, result)
			if result, err = s.Execer.RunCommandAndReturn(SystemCtl, "", "enable", s.id); err == nil {
				output = fmt.Sprintf("%s\n%s", output, result)
			}
		}
	}
...
func (s *podmanService) uninstallService() (output string, err error) {
	output, err = s.Execer.RunCommandAndReturn(SystemCtl, "", "disable", s.id)
	return
}
